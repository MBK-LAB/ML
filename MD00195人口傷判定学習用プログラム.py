...
#指定したフォルダの画像をすべて読み込む➡opencvで検出、輪郭描写、背景黒潰しを行う。　
for dir in os.listdir("/home/pi/data/train/"):
    ...
    for file in os.listdir(dir1):
        if file != ".DS_Store":
            ...
            cv2.waitKey(0)
            """
            image = cv2.resize(img_masked, (96, 96))		#画像の縮小
            cv2.imshow("gaisetu", image)
            cv2.waitKey(0)
...
for dir in os.listdir("/home/pi/data/val/"):
    ...
    for file in os.listdir(dir1):
        if file != ".DS_Store":
            ...
            #閾値処理
            _, val_image_binary = cv2.threshold(val_image_preprocessed, 40, 255, cv2.THRESH_BINARY)
            ...
            """
            val_image = cv2.resize(val_img_masked, (96, 96))		#画像の縮小
...
early_stopping = keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0.001,  patience=3, verbose=0, mode='auto')

history = model.fit(image_list, label_list, epochs=50, batch_size=30, validation_data=(val_image_list, val_label_list), callbacks=[early_stopping])
